...
import { OurTeamSection, ServiceTimesSection } from "./ScopeOfWorkTeam";
import {
  facilitySummary,
  recentVerifications,
  recentAudits,
...
export type ScopeOfWorkPageProps = {
  associates: RosterPerson[];
  managers: RosterPerson[];
  contractBuildings: ContractBuilding[];
};

export type ScopeOfWorkTab = "spaces" | "team" | "serviceTimes" | "charts";

const TABS: { id: ScopeOfWorkTab; label: string; icon: string }[] = [
...
/**
 * ScopeOfWorkPage — "Your Spaces / Our Team / Service Times"
 ...
 * fabricated number.
 */
export function ScopeOfWorkPage({ associates, managers, contractBuildings }: ScopeOfWorkPageProps) {
  const [activeTab, setActiveTab] = useState<ScopeOfWorkTab>("spaces");
  ...
  // ScopeOfWorkHero) rather than inside YourSpacesSection, so every card/
...
function ScopeOfWorkHero({
  ...
  activeTab: ScopeOfWorkTab;
  onTabChange: (tab: ScopeOfWorkTab) => void;
...
function YourSpacesSection({
  ...
  const filteredSortedAreaCards = useMemo(() => {
    ...
    return sorted;
  }, [individualAreaCards, selectedAreaType, q, buildingFilter, shiftFilter, sortOrder]);
      ...
      <section ref={areaTypesRef} style={{ display: "flex", flexDirection: "column", gap: 24 }}>
        ...
        {selectedAreaType && (
          <div className={styles.selectedChipRow}>
            ...
            <button type="button" className={styles.selectedChip} onClick={() => onSelectAreaType(null)}>
        ...
        {effectiveGroupBy === "buildings" ? (
          ...
          filteredSortedAreaCards.length === 0 ? (
            ...
            <AreaListView cards={filteredSortedAreaCards} />
          )
 ...
 */
function AreaListView({ cards }: { cards: AreaCardData[] }) {
  ...
  const compliantCount = cards.filter((c) => areaExpectedHit(c)).length;

  return (
    <div className={styles.areaTypeTableWrap}>
      <button type="button" className={styles.complianceSummaryToggle} onClick={() => setShowCompliance((v) => !v)}>
        {showCompliance ? "Hide" : "Show"} Compliance Summary
      </button>
      {showCompliance && (
        <p className={styles.complianceSummaryLine}>
          <strong>{compliantCount}</strong> of {cards.length} hit their Expected Services target today.
        </p>
      )}
...
function AreaTypeListView({
  ...
  const compliantCount = cards.filter((c) => areaTypeExpectedHit(c)).length;
